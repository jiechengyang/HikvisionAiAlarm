...
namespace BytAiAlarmService.Helper
{
    public class FileHelper
    {
        ...
        {
            if (!string.IsNullOrEmpty(varToDirectory))
            {
                Directory.CreateDirectory(varToDirectory);
            }
            ...
            if (directories.Length > 0)
            {
                foreach (string d in directories)
                {
                    DeleteFolderFiles(d, varFromDirectory + d.Substring(d.LastIndexOf("\\")));
                }
            }
            ...
            if (files.Length > 0)
            {
                foreach (string s in files)
                {
                    File.Delete(varFromDirectory + s.Substring(s.LastIndexOf("\\")));
                }
            }
        }
        #endregion
